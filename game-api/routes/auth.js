...
//bcrypt-비밀번호를 단방향 암호화(해시)하여 저장할 수 있도록 도와줌
...
router.post('/join', isNotLoggedIn, async (req, res, next) => {
   const { userId, userPassword, nick } = req.body
   try {
      //아이디로 기존 사용자 검색
      ...
      //비밀번호 암호화
      const hash = await bcrypt.hash(userPassword, 12) // 12: salt(해시 암호화를 진행시 추가되는 임의의 데이터로 10~12 정도의 값이 권장)
